...
fn test_explain_shows_command_change_reason() {
    ...
    let broskifile = fs::read_to_string(workspace.join("broskifile")).expect("read broskifile");
    let updated =
        broskifile.replace("cat src/input.txt > dist/out.txt", "cp src/input.txt dist/out.txt");
    fs::write(workspace.join("broskifile"), updated).expect("write broskifile");
...
fn test_explain_shows_env_change_reason() {
    ...
    let broskifile = fs::read_to_string(workspace.join("broskifile")).expect("read broskifile");
    let with_env = broskifile.replace(
        "@isolation best_effort\n    mkdir -p dist",
        "@isolation best_effort\n    @env MODE=a\n    mkdir -p dist",
    );
    fs::write(workspace.join("broskifile"), with_env).expect("write broskifile with env");

    support::broski_cmd(workspace).arg("run").arg("process").assert().success();

    let broskifile = fs::read_to_string(workspace.join("broskifile")).expect("read broskifile");
    let changed_env = broskifile.replace("MODE=a", "MODE=b");
    fs::write(workspace.join("broskifile"), changed_env).expect("write changed env");
